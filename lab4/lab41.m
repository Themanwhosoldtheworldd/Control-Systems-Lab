...
while (t < 5)
    ...
    xhatdot = Ahat*xhat + L*C*[1; x2] + B*u; %x1 can be whatever number we want because C = [0 1] it is put 1 instead of x1 for typical reasons (we cant measure the velocity)
    ...
    u=7;  %u and ControlSignal are the same thing
    controlSignal = 7; 
...
velocityData = [velocityData x1*KT];
controlData = [controlData controlSignal];
estimatedVelocityData = [estimatedVelocityData xhat(1)*KT];
